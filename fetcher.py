...
for cat, keywords in config['categories'].items():
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            if published_parsed:
                ...
                # 只取 48 小時內的新聞（放寬一點點確保有內容）
                if pub_time < (now - timedelta(days=2)): continue
    ...
    for n in news_results[cat]:
        if n['title'] not in seen:
            ...
            seen.add(n['title'])
    news_results[cat] = unique_news[:20] # 每個類別保留 20 則

# 最後存檔
with open('news_data.json', 'w', encoding='utf-8') as f:
    json.dump(news_results, f, ensure_ascii=False, indent=2)
print("抓取完成，請檢查 news_data.json")
